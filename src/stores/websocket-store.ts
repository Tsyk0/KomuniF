...
export const useWebSocketStore = defineStore('websocket', () => {
  ...
  /** 当前订阅的会话 ID（用于显示该会话在线人数） */
  const currentConvId = ref<number | null>(null);
  /** 当前会话在线人数（后端通过 conversationPresence 下发） */
  ...
  const connect = (userId: number, convId: number): Promise<void> => {
    return new Promise((resolve, reject) => {
      // 防止重复连接
      if (isConnecting) {
        console.log('[websocket-store] 已经在连接中，跳过');
        resolve();
        return;
      }
      ...
      try {
        // 从本地存储获取 access token，后续 WebSocket 需要认证
        const token = localStorage.getItem('access_token');
        if (!token) {
          isConnecting = false;
          connectionError.value = '未提供认证 token，请先登录';
          reject(new Error('未提供认证 token，请先登录'));
          return;
        }
        ...
        // 同源连接，通过查询参数携带 token（浏览器 WebSocket API 不支持自定义请求头）
        const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
        const wsUrl = `${protocol}//${window.location.host}/ws?token=${encodeURIComponent(token)}`;
        console.log('[websocket-store] 正在建立 WebSocket 连接:', wsUrl.replace(/token=[^&]+/, 'token=***'));

        const ws = new WebSocket(wsUrl);

        ws.onopen = () => {
          console.log('[websocket-store] WebSocket 连接已建立');
          ...
          startHeartbeat();
          // 建立连接后订阅当前会话，以便后端回传在线人数
          sendSubscribe(convId);
          resolve();
        };

        ws.onclose = (event) => {
          console.log('[websocket-store] WebSocket 连接已关闭:', event.code, event.reason);
        ...
        ws.onerror = (error) => {
          console.error('[websocket-store] WebSocket 连接错误:', error);
        ...
        console.error('[websocket-store] 建立 WebSocket 连接失败:', error);
  ...
  const disconnect = () => {
    if (wsConnection.value) {
      ...
      console.log('[websocket-store] 正在关闭 WebSocket 连接');
      console.log('  当前状态:', {
  ...
  const sendMessage = (message: WebSocketMessage): boolean => {
    console.log('[websocket-store] 尝试发送 WebSocket 消息');
    ...
    if (!wsConnection.value || readyState !== WebSocket.OPEN) {
      console.error('[websocket-store] WebSocket 连接不存在或未打开');
      console.error('  详细状态:', {
        hasConnection: !!wsConnection.value,
        readyState: readyState,
        readyStateText: getReadyStateText(readyState), // 传入 readyState
        storeIsConnected: isConnected.value
      });

      // 自动修复状态不一致
      if (isConnected.value && (!wsConnection.value || readyState !== WebSocket.OPEN)) {
        console.warn('[websocket-store] 状态不一致，自动修正');
        isConnected.value = false;
      }
    ...
    try {
      ...
      console.log('[websocket-store] 发送 WebSocket 消息:', messageStr);
      wsConnection.value.send(messageStr);
      console.log('[websocket-store] WebSocket 消息发送成功');
      return true;
    } catch (error) {
      console.error('[websocket-store] 发送 WebSocket 消息失败:', error);
      return false;
    }
  };
  ...
  const startHeartbeat = () => {
    ...
    heartbeatInterval = window.setInterval(() => {
      if (wsConnection.value?.readyState === WebSocket.OPEN) {
        console.log('[websocket-store] 发送心跳 ping');
  ...
  const handleWebSocketMessage = (data: string) => {
    console.log('[websocket-store] 收到 WebSocket 消息:', data);

    try {
      // 处理心跳响应（纯文本）
      if (data === 'pong') {
        console.log('[websocket-store] 收到心跳响应 pong');
        return;
      }
      ...
      try {
        ...
        // 如果不是 JSON，可能是纯文本消息
        console.log('[websocket-store] 收到非 JSON 消息:', data);
        return;
      }
      ...
      switch (message.action) {
        ...
        case 'conversationPresence':
          if (typeof message.convId === 'number' && typeof message.onlineCount === 'number') {
            if (message.convId === currentConvId.value) {
              conversationOnlineCount.value = message.onlineCount;
              console.log('[websocket-store] 当前会话在线人数:', message.onlineCount);
            }
          }
          break;
        case 'error':
          ...
          break;
        default:
          console.warn('[websocket-store] 未知的 WebSocket 消息类型:', message.action, message);
      }
    } catch (error) {
      console.error('[websocket-store] 解析 WebSocket 消息失败:', error, data);
    }
  };
  ...
  const handleNewMessage = (message: any) => {
    console.log('[websocket-store] 收到 newMessage 广播:', message);

    // 确保消息格式正确
    if (!message.convId || !message.messageContent) {
      console.warn('[websocket-store] 收到的消息格式不正确:', message);
      return;
    }
    ...
    console.log('[websocket-store] 派发 websocket:newMessage 事件');
    window.dispatchEvent(event);
  };
  ...
  const handleErrorMessage = (message: any) => {
    ...
    if (errorMsg && typeof errorMsg === 'string' && errorMsg.includes('ping')) {
      console.log('[websocket-store] 忽略 ping 相关的历史错误:', errorMsg);
      return;
    }

    console.error('[websocket-store] WebSocket 错误:', errorMsg);
  ...
  const sendTextMessage = (
    ...
    const message: SendMessageRequest = {
      ...
      // timestamp: Date.now(), // timestamp 不是 SendMessageRequest 的标准字段，但在 BaseMessage 里可选
  ...
  /**
   * 订阅当前会话（用于后端更新 currentConvId 并回传在线人数）
   */
  const sendSubscribe = (convId: number): boolean => {
    currentConvId.value = convId;
    conversationOnlineCount.value = null; // 清空直到收到 conversationPresence
    const msg = { action: 'subscribe', convId };
    console.log('[websocket-store] 发送 subscribe:', convId);
  ...
  const reconnect = async (): Promise<boolean> => {
    ...
    try {
      console.warn('[websocket-store] 手动重连需要 userId 和 convId 参数');
      return false;
    } catch (error) {
