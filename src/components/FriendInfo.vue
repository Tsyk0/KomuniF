...
const info = computed<FriendInfoViewModel | null>(() => {
  if (friendStore.currentFriend)
    return friendStore.currentFriend as FriendInfoViewModel;
...
const avatarUrl = computed(() =>
  normalizeAvatarUrl(
    String(
      (info.value as FriendInfoViewModel | null)?.avatar ||
...
const nicknameDisplay = computed(() => {
  ...
  return info.value.friendNickname || info.value.nickname || "未知用户";
});
...
const handleApplyRemarkGroup = async () => {
  const targetFriendId = Number(
    info.value?.friendId || props.friend?.friendId || 0
  );
  if (
